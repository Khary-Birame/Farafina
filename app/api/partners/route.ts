...
export async function OPTIONS() {
  console.log("[API Partners] OPTIONS request - CORS preflight")
  return NextResponse.json({}, { headers: corsHeaders })
}

export async function POST(request: NextRequest) {
  const startTime = Date.now()
  console.log("[API Partners] 🚀 POST request reçue")
  ...
  try {
    ...
    try {
      ...
      console.log("[API Partners] ✅ Body parsé avec succès")
    } catch (parseError: any) {
      console.error("[API Partners] ❌ Erreur parsing body:", parseError.message)
    ...
    if (!organizationName || !contactName || !email || !phone || !partnershipType || !message) {
      console.error("[API Partners] ❌ Champs obligatoires manquants")
      return NextResponse.json(
    ...
    if (!emailRegex.test(email)) {
      console.error("[API Partners] ❌ Email invalide:", email)
      return NextResponse.json(
    ...
    console.log("[API Partners] ✅ Validation des données réussie")
    ...
    if (!emailUser || !emailPass || !emailTo) {
      ...
      console.error("[API Partners] ❌ Variables d'environnement email manquantes:", missingVars.join(", "))
      return NextResponse.json(
        { 
    ...
    console.log("[API Partners] ✅ Configuration email vérifiée")
    ...
    console.log("[API Partners] 📧 Préparation des emails...")
    ...
    console.log("[API Partners] 📤 Envoi email admin...")
    ...
    try {
      ...
      console.log(`[API Partners] ✅ Email admin envoyé en ${emailElapsed}s`)
    } catch (emailError: any) {
      const emailElapsed = ((Date.now() - emailStartTime) / 1000).toFixed(2)
      console.error(`[API Partners] ❌ Erreur envoi email admin après ${emailElapsed}s:`, emailError.message)
    ...
    console.log("[API Partners] 📤 Envoi accusé de réception...")
    ...
    try {
      ...
      console.log(`[API Partners] ✅ Accusé de réception envoyé en ${ackElapsed}s`)
    } catch (ackError: any) {
      const ackElapsed = ((Date.now() - ackStartTime) / 1000).toFixed(2)
      console.error(`[API Partners] ❌ Erreur envoi accusé réception après ${ackElapsed}s:`, ackError.message)
      // On retourne quand même un succès car l'email admin a peut-être été envoyé
    }

    const totalElapsed = ((Date.now() - startTime) / 1000).toFixed(2)
    console.log(`[API Partners] ✅ Traitement terminé en ${totalElapsed}s`)
    ...
    const totalElapsed = ((Date.now() - startTime) / 1000).toFixed(2)
    console.error(`[API Partners] ❌ Erreur globale après ${totalElapsed}s:`, {
